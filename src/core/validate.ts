...
export function validatePack(pack: Pack): Issue[] {
  ...
  pack.entries.forEach((entry, index) => {
    ...
    for (const clue of entry.clues) {
      ...
      if (clue.quote !== undefined) {
        // Цитата уезжает в браузер вместе с кроссвордом. Ответ внутри неё —
        // это ответ открытым текстом в `public/p/<id>.json`, то есть пробитая
        // главная гарантия проекта. Поэтому ошибка, а не предупреждение.
        if (isCognate(answer, clue.quote)) {
          issues.push({ severity: 'error', where, message: `Цитата выдаёт ответ: «${clue.quote}»` })
        }
        // Цитата — свойство значения. У словарного значения книжной фразы нет:
        // если она там появилась, её приписали не тому определению.
        if (clue.kind === 'dict') {
          issues.push({ severity: 'error', where, message: `Цитата у словарного определения: «${clue.quote}»` })
        }
      }
